...
@Component({
  selector: 'SignInSimpleCard',
  imports: [MatCard, MatButton, MatFormField, MatInput, MatLabel, MatCheckbox, MatDivider, NgOptimizedImage],
  template: `
    <div class="flex flex-auto flex-col items-center justify-center p-6">
      <mat-card class="w-full max-w-sm px-8 py-12 sm:px-10">
        ...
        <div class="mt-8 flex items-center gap-x-4">
          <mat-divider class="flex-auto" />
          <div class="font-medium text-neutral-a11">Or continue with</div>
          <mat-divider class="flex-auto" />
        </div>

        <div class="mt-4 flex items-center gap-x-4">
          <button class="flex-auto" matButton>Google</button>
          <button class="flex-auto" matButton>GitHub</button>
        </div>
      </mat-card>
    </div>
  `,
})
export class SignInSimpleCard {}
