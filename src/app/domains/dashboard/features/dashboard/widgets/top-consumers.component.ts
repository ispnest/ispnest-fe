...
import { TopConsumersDialogComponent } from './top-consumers-dialog.component';

/**
 * Dashboard widget: top customers by usage since the start of today. Refetches when live
 * network-usage events arrive, throttled to at most one request per 30 s. "View all" opens a
 * paginated modal covering the full ranking beyond the inline top 8.
 */
@Component({
  ...
  imports: [
    ...
    BuiDialogFooter,
    BuiDialogClose,
    TopConsumersDialogComponent,
  ],
  template: `
    <div buiDialog #d="buiDialog">
      <mat-card class="flex h-full flex-col">
        <div class="flex items-center gap-x-2 border-b border-neutral-a4 px-4 py-3">
          ...
          <button matButton class="tertiary ml-auto" buiDialogTrigger>View all</button>
        </div>
      ...
      <ng-template buiDialogPortal>
        ...
        <div buiDialogContent class="max-w-2xl">
          <div buiDialogHeader>
            <h2 buiDialogTitle>Top Consumers Today</h2>
          </div>
          <div buiDialogBody>
            <app-top-consumers-dialog (rowSelected)="d.close()" />
          </div>
          <div buiDialogFooter>
